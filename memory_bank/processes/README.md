...
- [A Corpus Run That Only Asks "Did It Parse" Will Pass On A Book Of Replacement Characters](a-corpus-run-that-only-asks-whether-it-parsed.md) — Read before trusting a corpus run as evidence, or before adding a fixture for a case: the two mislabelled-encoding fixtures reported ok in every run for a day, because the runner asserted that parsing returned a value and never looked at the value.
