...
- [ADR-20260901T101500Z: Value Equality On Sentence And Word Only](ADR-20260901T101500Z-value-equality-on-spans-only.md) — Records which exported model types define ==, why every type above a sentence keeps identity, and why the answer had to be chosen before 0.1.0 rather than defaulted.
- [ADR-20260901T101600Z: ParseFailureKind Is Closed At Five, With drmProtected](ADR-20260901T101600Z-parse-failure-kinds-closed-at-five.md) — Records the final set of parse failure causes, why DRM earns its own kind, and why an encryption.xml alone must not be read as DRM.
- [ADR-20260901T101700Z: emptyDocument Means No Blocks, Not No Text](ADR-20260901T101700Z-empty-document-means-no-blocks.md) — Defines the emptyDocument failure, records that a chapter with no blocks is dropped, and says why an image-only book is a valid document rather than an error.
- [ADR-20260901T101800Z: The Codec Encodes Images By Reference, Not By Value](ADR-20260901T101800Z-images-encoded-by-reference.md) — Records why image bytes stay out of the encoded document, the measured sizes that forced it, and the encode/decode signatures that hand the bytes back to the caller.
- [ADR-20260901T101900Z: FB2 parseMetadata Streams Rather Than Building A DOM](ADR-20260901T101900Z-streaming-fb2-metadata.md) — Records that the FB2 metadata path is event-based so parseMetadata is cheap for both formats, and what the second FB2 reading path costs to maintain.
- [ADR-20260902T120000Z: A UTF-8 Declaration In FB2 Is A Hypothesis, Not A Fact](ADR-20260902T120000Z-fb2-encoding-declaration-is-a-hypothesis.md) — Records why a mislabelled or undeclared FB2 is re-decoded as legacy Cyrillic instead of returning a body of U+FFFD, how the two candidate codecs are told apart, and why refusing the file was rejected.
